"""Slice/reshape the raw per-deal eval data (eval_scaling.csv) for readable analysis.
    ...
    python -m imposterkings.eval_slice --n 200                 # per-seed table for N=200
    python -m imposterkings.eval_slice --n 200 --per-game      # 2 rows per seed (one per mirrored game)
    python -m imposterkings.eval_slice --sweeps                # who swept each deal, across all N
